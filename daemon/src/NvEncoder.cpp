...
Encoder::Encoder()
    ...
    , m_bIsHEVC(false)
    , m_bIsYUV444(false)
    , m_bIs10bitRGBCapture(false)
    , m_bEnableEmphasisLevelMap(false)
{
    //! Init members
...
HRESULT Encoder::SetupEncoder(unsigned int dwWidth, unsigned int dwHeight, unsigned int dwBitRate, unsigned int maxWidth, unsigned int maxHeight,
                                unsigned int codec, IDirect3DSurface9 **ppInputSurfs, bool bYUV444, bool b10bitRGBCapture,
                                bool bEnableEmphasisMap, BYTE* pEmphasisMap, bool bConstQP, unsigned int QP)
{
    if (!m_hEncoder)
        return E_FAIL;
    ...
    m_bIsHEVC = !!codec;
    m_bIsYUV444 = bYUV444;
    m_bIs10bitRGBCapture = b10bitRGBCapture;
    ...
    m_dwInputFormat = m_bIs10bitRGBCapture ? NV_ENC_BUFFER_FORMAT_ARGB10 : NV_ENC_BUFFER_FORMAT_ARGB;
    if (m_bIs10bitRGBCapture)
    {
        if (!m_bIsHEVC)
        {
            printf("Warning! 10-bit ARGB capture also requires HEVC encoding with with Main 10 or Main 4:4:4 10 profile\n");
        }
    }
    ...
    if (bConstQP)
    {
        ...
        m_stEncodeConfig.rcParams.constQP.qpIntra  = QP;
        m_stEncodeConfig.rcParams.constQP.qpInterP = QP;
        m_stEncodeConfig.rcParams.constQP.qpInterB = QP;
    }
    else
    {
        m_stEncodeConfig.rcParams.rateControlMode  = NV_ENC_PARAMS_RC_CBR_LOWDELAY_HQ;
    ...
    //! Enable emphasis level map
    if (bEnableEmphasisMap)
    {
        //! Let NvEncodeAPI know that it should interpret the QPMap buffer as Emphasis Level map
        m_bEnableEmphasisLevelMap = bEnableEmphasisMap;
        m_stEncodeConfig.rcParams.qpMapMode = NV_ENC_QP_MAP_EMPHASIS;
        m_pEmphasisMap = pEmphasisMap;
    }
    ...
    if (m_bIsHEVC)
    {
        ...
        // set up if 10bit encoding requested.
        m_stEncodeConfig.encodeCodecConfig.hevcConfig.pixelBitDepthMinus8 = m_bIs10bitRGBCapture ? 2 : 0;
    }
    else
    {
...
HRESULT Encoder::Reconfigure(unsigned int dwWidth, unsigned int dwHeight, unsigned int dwBitRate)
{
    if (!m_hEncoder)
        return E_FAIL;
...
HRESULT Encoder::LaunchEncode(unsigned int i)
{
    ...
    if (m_bEnableEmphasisLevelMap)
    {
        m_stEncodePicParams.qpDeltaMap = (int8_t*)m_pEmphasisMap;
        m_stEncodePicParams.qpDeltaMapSize = (int)(ceil((float)m_stEncodePicParams.inputWidth / 16) * ceil((float)m_stEncodePicParams.inputHeight / 16));
    }
...
HRESULT Encoder::GetBitstream(unsigned int i)
{
    if (!m_hEncoder)
        return E_FAIL;
    HRESULT hr = S_OK;
...
void Encoder::SetOnEncoded(onEncode_fp fp)
{
	onEncoded = fp;
}
